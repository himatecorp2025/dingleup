...
const Gifts = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-900 via-purple-800 to-purple-900 text-white relative overflow-hidden pb-24">
      {/* Background Effects */}
      ...
      <div className="relative z-10 container mx-auto px-4 py-6 max-w-2xl">
        {/* Header */}
        <h1 className="text-3xl font-black text-center mb-8 text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 drop-shadow-[0_2px_8px_rgba(234,179,8,0.6)]">
          {t('gifts.title')}
        </h1>

        {/* My Reward Boxes Section */}
        <div className="mb-12">
          <h2 className="text-xl font-bold mb-4 text-yellow-400 drop-shadow-[0_2px_4px_rgba(234,179,8,0.4)]">
            {t('gifts.my_boxes')}
          </h2>
            ...
            {Array.from({ length: 10 }).map((_, index) => (
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-xs font-bold text-yellow-400/60">{t('gifts.inactive')}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {packages.map((pkg, index) => (
              <div
                key={index}
                className="relative rounded-xl p-4 backdrop-blur-sm transform-gpu hover:scale-105 transition-transform cursor-pointer"
              >
                {/* 3D Frame Effects */}
                ...
                <div className="relative z-10 flex flex-col items-center">
                  <img src={boxGold} alt="Gift box" className="w-20 h-20 object-contain mb-2" />
                  <p className="text-2xl font-black text-yellow-400 mb-1">{pkg.boxes} {pkg.boxes === 1 ? t('gifts.box_singular') : t('gifts.box_plural')}</p>
                  <p className="text-sm text-yellow-300/80 mb-2">{t('gifts.random_rewards')}</p>
                  <p className="text-xl font-bold text-white mb-3">{pkg.price}</p>
                  <button className="w-full py-2 px-4 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 rounded-lg font-bold text-black shadow-lg transition-all">
                    {t('gifts.buy')}
                  </button>
                </div>
              </div>
