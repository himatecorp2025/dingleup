...
const DevelopmentStatus = () => {
  ...
  const statusItems = [
    {
      category: t('landing.status.category_ux'),
      progress: 85,
      color: "from-accent to-accent",
      description: t('landing.status.category_ux_desc')
    },
    {
      category: t('landing.status.category_game'),
      progress: 90,
      color: "from-secondary to-primary",
      description: t('landing.status.category_game_desc')
    },
    {
      category: t('landing.status.category_social'),
      progress: 60,
      color: "from-accent to-accent-dark",
      description: t('landing.status.category_social_desc')
    },
    {
      category: t('landing.status.category_testing'),
      progress: 75,
      color: "from-success to-success",
      description: t('landing.status.category_testing_desc')
    }
  ];
  ...
  const comingSoonFeatures = [
    {
      icon: Crown,
      title: t('landing.status.feature_vip_title'),
      description: t('landing.status.feature_vip_desc'),
    ...
    },
    {
      ...
      badge: t('landing.status.badge_planned')
    },
    {
      icon: Shield,
      title: t('landing.status.feature_avatars_title'),
      description: t('landing.status.feature_avatars_desc'),
      badge: t('landing.status.badge_soon')
    }
  ];
      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="text-center mb-16 animate-fade-in">
          <div className="inline-flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-full px-6 py-3 mb-6 backdrop-blur-sm">
            ...
            <span className="text-sm sm:text-base font-bold text-yellow-300">{t('landing.status.badge_active')}</span>
          </div>
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-black mb-6 text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.3)]">
            {t('landing.status.title_part1')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-yellow-400 to-amber-500">{t('landing.status.title_part2')}</span>
          </h2>
        ...
        <div className="text-center mb-12 animate-fade-in">
          <div className="inline-flex items-center gap-2 bg-secondary/10 border border-secondary/30 rounded-full px-4 py-2 mb-4">
            ...
            <span className="text-sm font-semibold text-secondary">{t('landing.status.upcoming_badge')}</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold mb-4 font-poppins text-white">
            {t('landing.status.upcoming_title_part1')} <span className="text-white">{t('landing.status.upcoming_title_part2')}</span>
          </h2>
        </div>
