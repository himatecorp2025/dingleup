    ...
    <div className="relative mx-2 mb-4">
      <div className="relative rounded-3xl p-1" style={{ background: 'linear-gradient(135deg, hsl(var(--dup-gold-700)), hsl(var(--dup-gold-500)), hsl(var(--dup-gold-700)))' }}>
        <div className="relative rounded-3xl p-5" style={{ background: 'linear-gradient(135deg, hsl(var(--dup-purple-900)), hsl(var(--dup-purple-800)))' }}>
          <h2 className="text-2xl font-black text-center mb-4" style={{ background: 'linear-gradient(135deg, hsl(45 100% 70%), hsl(45 100% 60%))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{t('daily_rewards.geniuses_title')}</h2>
          <div className="space-y-3">
            ...
            {topPlayers.slice(0, maxRank).map((player, index) => {
              ...
              const isTop3 = place <= 3;
              return (
                <div key={index} className="relative rounded-xl p-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl overflow-hidden" 
                  style={{
                    background: isTop3 
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex flex-col min-w-0 flex-1">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="text-2xl font-black flex-shrink-0" style={{ color: isTop3 ? (place === 1 ? 'hsl(45 100% 88%)' : place === 2 ? 'hsl(0 0% 97%)' : 'hsl(30 78% 88%)') : 'hsl(var(--dup-gold-400))', textShadow: isTop3 ? `0 3px 6px rgba(0, 0, 0, 0.6), 0 0 ${place === 1 ? '15px' : '10px'} ${place === 1 ? 'rgba(255, 215, 0, 0.6)' : 'rgba(255, 255, 255, 0.4)'}` : '0 2px 4px rgba(0, 0, 0, 0.5)' }}>#{place}</span>
                          <span className="text-lg font-bold text-white truncate overflow-hidden whitespace-nowrap" style={{ textShadow: '0 2px 4px rgba(0, 0, 0, 0.6)', textOverflow: 'ellipsis', maxWidth: '140px' }}>{player.username}</span>
                        </div>
                        <span className="text-xs ml-10 font-semibold" style={{ color: isTop3 ? 'rgba(255, 255, 255, 0.85)' : 'hsl(45 85% 68%)', textShadow: '0 1px 3px rgba(0, 0, 0, 0.5)' }}>
                          {t('daily_rewards.correct_answers').replace('{count}', String(player.total_correct_answers))}
                        </span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-2 flex-shrink-0" style={{ minWidth: '100px' }}>
                      <div className="flex items-center justify-end gap-2 w-full">
                        ...
                        <span className="text-base font-black tabular-nums" style={{ color: 'hsl(45 100% 80%)', textShadow: '0 2px 6px rgba(0, 0, 0, 0.7), 0 0 12px rgba(255, 215, 0, 0.5)', minWidth: '65px', textAlign: 'right' }}>+{reward.coins.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center justify-end gap-2 w-full">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ filter: 'drop-shadow(0 2px 4px rgba(0, 0, 0, 0.4))' }}>
                        ...
                        <span className="text-base font-black tabular-nums" style={{ color: 'hsl(0 88% 75%)', textShadow: '0 2px 6px rgba(0, 0, 0, 0.7), 0 0 12px rgba(255, 0, 0, 0.4)', minWidth: '65px', textAlign: 'right' }}>+{reward.lives}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="mt-6 text-center">
        <p className="text-sm font-semibold" style={{ color: 'hsl(45 100% 75%)' }}>{t('daily_rewards.midnight_info')}</p>
      </div>
    </div>
  );
};
