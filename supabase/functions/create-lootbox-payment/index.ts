...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { priceId, boxes } = await req.json();

    if (!priceId || !boxes) {
      return new Response(JSON.stringify({ error: "Missing priceId or boxes" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 400,
      });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/gifts?payment=success&boxes=${boxes}`,
    ...
    console.error('[create-lootbox-payment] Error:', error);
