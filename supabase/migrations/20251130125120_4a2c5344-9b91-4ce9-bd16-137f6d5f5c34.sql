-- Add day_of_week column to daily_prize_table (idempotent)
ALTER TABLE public.daily_prize_table 
ADD COLUMN IF NOT EXISTS day_of_week INTEGER NOT NULL DEFAULT 1;

-- Add constraint for day_of_week (1-7) - idempotent
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'day_of_week_range' 
    AND conrelid = 'public.daily_prize_table'::regclass
  ) THEN
    ALTER TABLE public.daily_prize_table 
