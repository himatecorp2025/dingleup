-- Fix claim_daily_gift to return server-authoritative values
...
BEGIN
  ...
  INTO last_claimed, current_streak, is_subscriber
  ...
  base_coins := reward_amounts[new_streak];
  
  -- Double reward for Genius members
  IF is_subscriber THEN
    granted_coins := base_coins * 2;
  ELSE
    granted_coins := base_coins;
  END IF;
  
  -- Use credit_wallet for idempotent operation
  SELECT credit_wallet(
    user_id,
    granted_coins,  -- coins (already doubled if Genius)
    0,              -- lives
    'daily',
    'daily:' || user_id::text || ':' || today::text,
    json_build_object('streak', new_streak, 'date', today, 'base_coins', base_coins, 'genius_bonus', is_subscriber)::jsonb
  ) INTO credit_result;
  ...
  -- Extract new balance from credit_result
  new_balance := (credit_result->>'new_coins')::integer;
