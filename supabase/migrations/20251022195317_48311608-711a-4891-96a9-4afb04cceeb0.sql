-- Fix remaining security issues (handle existing policies)
...
DROP POLICY IF EXISTS "Service role can manage rankings" ON public.weekly_rankings;
...
DROP POLICY IF EXISTS "Service can aggregate activity data" ON public.user_activity_daily;
...
-- Update purchase_life with fixed search_path
...
-- Update regenerate_lives_background with fixed search_path
